...
func InsertProduct(p models.Product) (int64, error) {
	...
	fmt.Println("InserProducts > EJECUCION EXITOSA")

	return LastInsertId, nil
}

func UpdateProducts(p models.Product) error {
...
func SelectProducts(p models.Product, choice string, page int, pageSize int, orderType string, orderField string) (models.ProductResp, error) {
	fmt.Println("Comienza SelectProducts")
	...
	case "S":
		where = " WHERE UCASE(CONCAT(COALESCE(Prod_Title, ''), COALESCE(Prod_Description, ''))) LIKE '%" + strings.ToUpper(p.ProdSearch) + "%' "
	case "C":
	...
	rows.Next() // Muevo el cursor al primer registro
